  ...
  return (
    <Sheet open={open} onClose={onClose}>
      <div className="flex items-center justify-between px-6 pt-6 pb-5 md:px-8 md:pt-8">
        <div>
          <h2 className="font-headline text-xl italic font-light tracking-tight text-white md:text-2xl">
            Add {photoCount === 1 ? 'photo' : `${photoCount} photos`} to...
          </h2>
          <p className="mt-2 max-w-sm font-body text-xs leading-relaxed text-white/56 md:text-sm">
      ...
      <div className="flex-1 overflow-y-auto px-6 md:px-8">
        {showNewAlbumInput ? (
          <div className="mb-6 flex items-center gap-3">
            <input
              ...
              className="flex-1 rounded border border-white/10 bg-white/5 px-4 py-3 text-white label uppercase tracking-widest outline-none placeholder:text-white/20"
            />
            <button
          ...
          <button
            ...
            className="mb-6 flex min-h-[56px] w-full items-center rounded border border-dashed border-rose-accent/40 px-5 transition-colors hover:bg-white/5"
          >
        ...
        <div className="space-y-3 pb-6">
          {albums.map((album) => {
            ...
            return (
              <button
                ...
                className="flex h-[60px] w-full items-center justify-between rounded px-1 py-2 transition-colors hover:bg-white/3"
              >
                <div className="flex items-center gap-4">
                  <div
                    className={cn(
                      'flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-sm border transition-colors',
                      isChecked ? 'border-rose-accent bg-rose-accent' : 'border-white/20'
                    )}
                  >
                    {isChecked && (
                  ...
                  <div className="text-left">
                    <p className="font-headline text-lg leading-tight text-white">{album.title}</p>
          ...
          {albums.length === 0 && (
            <div className="rounded border border-white/6 bg-white/[0.02] px-4 py-4">
      ...
      <div className="border-t border-white/5 px-6 py-5 md:px-8">
        <button
          onClick={onSubmit}
          ...
          className={cn(
            ...
            selectedAlbumIds.length > 0 ? 'bg-rose-accent text-white' : 'bg-white/10 text-white/30'
          )}
        >
